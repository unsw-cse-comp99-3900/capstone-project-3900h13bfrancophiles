...
export async function checkInBooking(
  ...
  try {
    ...
    if (currentBookings.length != 1) {
      res
        .status(403)
        .json({ error: "Booking id does not exist for this user" });
      return;
    }
...
export async function checkOutBooking(
  ...
  try {
    ...
    if (
      !withinDateRange(
        currentTime,
        new Date(currentBooking[0].starttime),
        new Date(currentBooking[0].endtime),
        5
      )
    ) {
      res.status(403).json({ error: "Outside booking time window" });
...
export async function createBooking(
  ...
  if (status === null) {
    res
      .status(403)
      .json({ error: "You do not have permission to book this space" });
    return;
  }
...
export async function editBooking(
  ...
  try {
    ...
    if (existingBooking.length != 1) {
      res
        .status(404)
        .json({ error: "User does not have a booking with this id" });
      return;
    }
    ...
    const newBookingStatus = await initialBookingStatus(
      req.token.group,
    ...
    if (newBookingStatus === undefined) {
      res
        .status(404)
        .json({ error: `Space ${existingBooking[0].spaceid} not found` });
      return;
    }
    ...
    try {
      const res = await db
        ...
        .where(
          and(eq(booking.id, req.body.id), eq(booking.zid, req.token.user))
        )
        .returning();
